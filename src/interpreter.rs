...
impl Interpreter {
    ...
    fn syscall(&mut self) -> Result<(), i32> {
        match self.registers[2] {
            1 => {
                // Print Integer
                print!("{}", self.registers[4].cast_signed());
                Ok(())
            }
            4 => {
                // Print String
    ...
    fn execute_rtype(
        ...
        match funct {
            ...
            0x02 // srl
                => self.registers[rd] = self.registers[rs] >> shift,
            0x03 // sra
                => self.registers[rd] = (self.registers[rs].cast_signed() >> shift).cast_unsigned(),
            0x04 // sllv
                => self.registers[rd] = self.registers[rs] << (self.registers[rt] & 0b11111),
            0x06 // srlv
                => self.registers[rd] = self.registers[rs] >> (self.registers[rt] & 0b11111),
            0x07 // srav
                => self.registers[rd] = (self.registers[rs].cast_signed() >> (self.registers[rt] & 0b11111)).cast_unsigned(),
            0x08 // jr
            ...
            0x18 // mult
            => {
                let res = (self.registers[rs].cast_signed() as i64 * self.registers[rt].cast_signed() as i64).cast_unsigned();
                self.reg_hi = u32::try_from(res >> 32).unwrap();
                self.reg_lo = res as u32;
            }
            0x19 // multu
            => {
                let res = self.registers[rs] as u64 * self.registers[rt] as u64;
                self.reg_hi = u32::try_from(res >> 32).unwrap();
                self.reg_lo = res as u32;
            }
            0x1A // div
            => {
                self.reg_hi = (self.registers[rs].cast_signed() % self.registers[rt].cast_signed()).cast_unsigned();
                self.reg_lo = (self.registers[rs].cast_signed() / self.registers[rt].cast_signed()).cast_unsigned();
            }
            0x1B // divu
            => {
            ...
            0x20 // add
                => self.registers[rd] = (self.registers[rs].cast_signed() + self.registers[rt].cast_signed()).cast_unsigned(),
            0x21 // addu
            ...
            0x27 // nor
                => self.registers[rd] = !(self.registers[rs] | self.registers[rt]),
            0x2a // slt
                => self.registers[rd] = ((self.registers[rs].cast_signed()) < (self.registers[rt].cast_signed())) as u32,
            0x2b // sltu
    ...
    fn execute_itype(
        ...
        match opcode {
            0x01 if rd == 0 // bltz
                => if (src.cast_signed()) <  0 {self.pc = self.pc.checked_add_signed((immediate.cast_signed() << 2) as i32).ok_or(-1)?},
            0x01 if rd == 1 // bgez
                => if (src.cast_signed()) >= 0 {self.pc = self.pc.checked_add_signed((immediate.cast_signed() << 2) as i32).ok_or(-1)?},
            0x04 // beq
                => if src == *dst {self.pc = self.pc.checked_add_signed((immediate.cast_signed() << 2) as i32).ok_or(-1)?},
            0x05 // bne
                => if src != *dst {self.pc = self.pc.checked_add_signed((immediate.cast_signed() << 2) as i32).ok_or(-1)?},
            0x06 // blez
                => if (src.cast_signed()) <= 0 {self.pc = self.pc.checked_add_signed((immediate.cast_signed() << 2) as i32).ok_or(-1)?},
            0x07 // bgtz
                => if (src.cast_signed()) >  0 {self.pc = self.pc.checked_add_signed((immediate.cast_signed() << 2) as i32).ok_or(-1)?},
            0x08 // addi
                => *dst = (src.cast_signed() + immediate.cast_signed() as i32).cast_unsigned(),
            0x09 // addiu
                => *dst = src + immediate as u32,
            0x0a // slti
                => *dst = (src.cast_signed() <(immediate.cast_signed() as i32)) as u32,
            0x0b // sltiu
